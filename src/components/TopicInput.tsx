...
interface TopicInputProps {
  onAddTopics: (topics: string[], blogCount: number) => void;
  isGenerating: boolean;
}

export const TopicInput: React.FC<TopicInputProps> = ({ onAddTopics, isGenerating }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (topics.length > 0 || currentTopic.trim()) {
      const allTopics = currentTopic.trim() ? [...topics, currentTopic.trim()] : topics;
      onAddTopics(allTopics, blogCount);
      setTopics([]);
      setCurrentTopic('');
    }
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="blogCount" className="block text-sm font-medium text-gray-700 mb-2">
          ...
          <input
            ...
            id="blogCount"
            value={blogCount}
            onChange={(e) => setBlogCount(Math.max(1, Number(e.target.value) || 1))}
            min="1"
            max="50"
          ...
          <p className="text-xs text-gray-500 mt-1">
            Total blogs to generate: {(topics.length + (currentTopic.trim() ? 1 : 0)) * blogCount} (Max recommended: 20 per session)
          </p>
        </div>

        {topics.length > 0 && (
          <div>
            <h3 className="text-sm font-medium text-gray-700 mb-3">Topics to generate:</h3>
            ...
            <p className="text-xs text-gray-500 mt-2">
              {blogCount} blog{blogCount !== 1 ? 's' : ''} will be generated for each topic
            </p>
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={topics.length === 0 && !currentTopic.trim() || isGenerating}
        ...
        >
          {isGenerating ? 'Generating Blogs...' : `Generate ${(topics.length + (currentTopic.trim() ? 1 : 0)) * blogCount} Blog${(topics.length + (currentTopic.trim() ? 1 : 0)) * blogCount !== 1 ? 's' : ''}`}
        </button>
      </form>
    </div>
  );
